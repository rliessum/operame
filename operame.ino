...
static bool have_location = true;
...
static bool geolocate(float &latitude, float &longitude, float &accuracy)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (WiFi.SSID(i).endsWith("_nomap"))
        {
            continue;
        }
...
void calibrate()
{
    ...
    for (int count = 60; count >= 0; count--)
    {
        ...
        {
            if (button(pin_demobutton) || button(pin_portalbutton))
                return;
        }
    }
...
void ppm_demo()
{
    ...
    for (int p = 400; p < 1200; p++)
    {
        ...
        {
            while (!digitalRead(pin_portalbutton))
                delay(100);
            calibrate();
            display_logo();
            delay(500);
...
void setup_ota()
{
    ...
    ArduinoOTA.onStart([]()
                       { display_big("OTA", TFT_BLUE); });
    ArduinoOTA.onEnd([]()
                     { display_big("OTA done", TFT_GREEN); });
    ArduinoOTA.onError([](ota_error_t e)
                       { display_big("OTA failed", TFT_RED); });
...
{
    if (mqtt.connected())
        return; // already/still connected
...
int aqc_get_co2()
{
    ...
    if (!initialized && (co2 == 9999 || co2 == 400))
        return 0;
    initialized = true;
    return co2;
}
...
int mhz_get_co2()
{
    ...
    if (unclamped == mhz_co2_init && co2 - unclamped >= 10)
        return 0;

    // No known sensors support >10k PPM (library filter tests for >32767)
    if (co2 > 10000 || unclamped > 10000)
        return 0;
...
void setup()
{
    ...
    pm_enabled = WiFiSettings.checkbox("operame_pm", false, T.config_pm) && wifi_enabled;
    ...
    if (period != period_last)
    {
        ...
        if (have_location)
        {
            ...
            if (fetch_pm(latitude, longitude, "pm2.5", pm2_5))
            {
                ...
                printf("PM2.5=%f µg/m³, lat=%f, lon=%f\n", pm2_5, latitude, longitude);
                // Serial.printf("%d µg/m³", pm2_5);

                if(pm2_5 <= 50) {
                    printf("PM2.5=%f µg/m³, Green\n", pm2_5);
                    display_big(String(pm2_5), TFT_WHITE, TFT_DARKGREEN);
                } else if(pm2_5 >= 51 <= 100) {
                    printf("PM2.5=%f µg/m³, Yellow\n", pm2_5);
                    display_big(String(pm2_5), TFT_YELLOW);
                } else if(pm2_5 >= 101 <= 150) {
                    printf("PM2.5=%f µg/m³, Red\n", pm2_5);
                    display_big(String(pm2_5), TFT_ORANGE);
                } else if(pm2_5 >= 151 <= 200) {
                    printf("PM2.5=%f µg/m³, Red\n", pm2_5);
                    display_big(String(pm2_5), TFT_RED);
                } else if(pm2_5 >= 201 <= 300) {
                    printf("PM2.5=%f µg/m³, Red\n", pm2_5);
                    display_big(String(pm2_5), TFT_PURPLE);
                } else {
                    printf("PM2.5=%f µg/m³, Purple\n", pm2_5);
                    display_big(String(pm2_5), TFT_MAROON);
                }
                String pm25 = String(pm2_5);
                // String pm25 = String("%d µg/m³",pm2_5);
              
                // display_big(pm25, TFT_BLUE);
                delay(3000);
                return pm2_5;
            }
            else
